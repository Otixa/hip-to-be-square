...
public class CameraController : MonoBehaviour {
    ...
    void Start () {
		player = FindObjectOfType<GenericPlayer> ();		    //instantiate the GenericPlayer Object
		previousPlayerPosition = player.transform.position;	////we use a vector 3 as we will need to use 3D coords for the camera object, even if only using 2D
		...
		transform.position = new Vector3(player.transform.position.x + viewOffset, transform.position.y, transform.position.z);
	}
		
	void Update () {	
		if (!endOfLevel) {																			//how we want the camera to behave whilst the game is running
			distanceMoved = player.transform.position.x - previousPlayerPosition.x;	//work out how far camera needs to move, being how far the character has moved since last frame
			if (player.transform.position.y > heightToStartPan) { 						//if you are towards top of screen, meaning the camera should start scrolling
			...
			previousPlayerPosition = player.transform.position;						// this variable stores the current position, so you can compare next frame to calculate how far player has moved (and then move the camera that much)
		} else {
